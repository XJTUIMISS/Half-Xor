...
unsigned * hll_fd_generate_sketch(int m, unsigned long long n, int w, unsigned long long * dataset)
{
	unsigned * hll_fd_sketch = new unsigned[m * w]();
	unsigned long long i, j;
	int index_j = 0, m_bits = 0, index_i = 0;
	m_bits = log2(m);
	unsigned long long hash_outcome[2] = {0};
	for (i = 0; i < database_rows; ++i)
	{
		MurmurHash3_x64_128(&dataset[i], 8, hll_fd_seed, hash_outcome);
		index_j = rank_bits(hash_outcome[0], w);
		index_i = (hash_outcome[1] >> (64 - m_bits));
		hll_fd_sketch[index_i * w + index_j] += 1;
	}
	return hll_fd_sketch;
}
...
double hll_fd_estimate(int m, int w, unsigned * hll_fd_sketch)
{
...
int main()
{
	...
	unsigned * hll_fd_sketch = hll_fd_generate_sketch(m, n, w, dataset);
	estimate = hll_fd_estimate(m, w, hll_fd_sketch);
	cout << endl << setw(35) << "real_NDV:" << setw(20) << n << endl;
	cout << setw(35) << "storage:" << setw(20) << "8KB" << endl;
	cout << setw(35) << "w:" << setw(20) << w << endl << endl;
	cout << setw(35) << "HLL-FD" << setw(20) << estimate << endl << endl;
	delete[] hll_fd_sketch;
	delete[] dataset;
	return 0;
}